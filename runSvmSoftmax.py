...
# before setting up these two sets, I tried different lr and rs by hands.
# it turns out that lr=5e-8 & rs=4.8e4 give good acc
# so I expand the set around these two values.
learning = [4.5e-8, 4.6e-8, 4.7e-8, 4.8e-8, 4.9e-8, 5e-8, 5.1e-8, 5.2e-8, 5.3e-8, 5.4e-8]
reg_strength = [4e4, 4.2e4, 4.4e4, 4.6e4, 4.8e4, 5e4, 5.2e4, 5.4e4, 5.6e4, 5.8e4]

lr_history = -1
rs_history = -1

# use random search to find the best model
# here I try 10 pairs, can adjust it to larger number
for _ in range(10):
    lr_index = random.randint(0, 9)
    rs_index = random.randint(0, 9)
    # if current pair is used before, regenerate rs
    while lr_index == lr_history and rs_index == rs_history:
        rs_index = random.randint(0, 9)

    lr_history = lr_index
    rs_history = rs_index

    lr = learning[lr_index]
    rs = reg_strength[rs_index]

    classifier = Svm(xTrain.shape[1], numClasses)
    classifier.train(xTrain, yTrain, lr, rs, iter=1500, verbose=True)
    valAcc = classifier.calAccuracy(xVal, yVal)
    # if new acc is better, save it to best
    if valAcc > bestAcc:
        bestAcc = valAcc
...
# before setting up these two sets, I tried different lr and rs by hands.
# it turns out that lr=0.7e-6 & rs=6.8e3 give good acc
# so I expand the set around these two values.
learning = [0.2e-6, 0.3e-6, 0.4e-6, 0.5e-6, 0.6e-6, 0.7e-6, 0.8e-6, 0.9e-6, 1e-6, 1.1e-6]
reg_strength = [6e3, 6.2e3, 6.4e3, 6.6e3, 6.8e3, 7e3, 7.2e3, 7.4e3, 7.6e3, 7.8e3]
